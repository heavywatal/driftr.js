...
export function wrightFisherHaploid(
  N: number,
  s: number,
  q0: number,
  T: number,
...
export function wrightFisherDiploid(
  N: number,
  s: number,
  q0: number,
  T: number,
...
export function heterozygoteAdvantage(
  N: number,
  s: number,
  q0: number,
  T: number,
...
export function moranHaploid(
  ...
  for (let t = 1; t <= T * N; ++t) {
    ...
    if (randomBernoulli(Nq / N)()) {
      // a mutant dies
      if (!randomBernoulli(pMutRep)()) {
        --Nq;
      }
    } else {
      // a wildtype dies
