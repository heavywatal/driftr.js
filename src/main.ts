...
(() => {
  ...
  const panelHeight =
    parseInt(svg.style("height"), 10) - svgPadding.top - svgPadding.bottom;
  const plot = svg
    .append("g")
    .attr("class", "plot")
  ...
  const scaleY = d3.scaleLinear().domain([0, 1]).range([panelHeight, 0]);
  ...
  function updateWidth() {
    const plotWidth = parseInt(d3.select(".plot").style("width"), 10);
    ...
    svg
      .select(".title.x")
      .attr("transform", `translate(${panelWidth / 2},${panelHeight + 50})`);
